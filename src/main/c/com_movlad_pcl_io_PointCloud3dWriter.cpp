#include "com_movlad_pcl_io_PointCloud3dWriter.h"
...
void Java_com_movlad_pcl_io_PointCloud3dWriter_writePCD(JNIEnv *env, jobject obj, jstring file_name, jobject cloud, jboolean binary_mode)
{
...
void Java_com_movlad_pcl_io_PointCloud3dWriter_writePLY(JNIEnv *env, jobject obj, jstring file_name, jobject cloud, jboolean binary_mode)
{
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_ptr = sptr_wrapper<pcl::PointCloud<pcl::PointXYZRGB>>::get_sptr(env, cloud);
	const char *native_file_name = env->GetStringUTFChars(file_name, JNI_FALSE);
